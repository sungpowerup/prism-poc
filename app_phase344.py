"""
app_phase344.py
PRISM Phase 3.4.4 - Hybrid Detection (Streamlit UI) - 최종 정밀 조정
...
try:
    ...
    from core.layout_detector_v344 import LayoutDetectorV344
    from core.vlm_service import VLMService
    from core.storage import Storage
    from core.phase344_pipeline import Phase344Pipeline
...
# ============================================================
st.set_page_config(
    page_title="PRISM Phase 3.4.4 - Final Tuning",
    page_icon="🎯",
    layout="wide"
)
...
# 헤더
# ============================================================
st.markdown('<div class="main-header">🎯 PRISM Phase 3.4.4<span class="phase-badge">Final Tuning</span></div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">막대 3개 이상 | 원그래프 페이지별 필터 | 텍스트 비활성화 | 표 10~65%</div>', unsafe_allow_html=True)
...
try:
    if st.session_state.pipeline is None or st.session_state.get('current_provider') != vlm_provider:
        with st.spinner(f"🔄 {vlm_provider} 초기화 중..."):
            ...
            layout_detector = LayoutDetectorV344()
            vlm_service = VLMService(provider=vlm_provider)
            storage = Storage()
            
            st.session_state.pipeline = Phase344Pipeline(
            ...
            st.success(f"✅ Phase 3.4.4 파이프라인 초기화 완료: {vlm_provider}")
...
if uploaded_file is not None:
    if st.button("🚀 Phase 3.4.4 처리 시작", type="primary", use_container_width=True):
        try:
            # 임시 파일 저장
            temp_dir = Path("temp")
            ...
            # Pipeline 실행
            logger.info(f"🚀 Phase 3.4.4 처리 시작: {uploaded_file.name}")
...
if st.session_state.processing_result is not None:
    ...
    with col2:
        # Markdown 다운로드
        md_content = f"""# PRISM Phase 3.4.4 처리 결과
...
<div style='text-align: center; color: #888; font-size: 0.9rem;'>
    <strong>PRISM Phase 3.4.4 - Final Tuning</strong><br>
    🎯 막대 3개 이상 | 원그래프 페이지별 필터 | 텍스트 비활성화 | 표 10~65%<br>
