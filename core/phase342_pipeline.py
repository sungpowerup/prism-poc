"""
core/phase342_pipeline.py
PRISM Phase 3.4.2 - Hybrid Detection Pipeline (정밀 조정)

✅ Phase 3.4.2 수정사항:
- LayoutDetectorV342 사용 (v3.4.1 → v3.4.2)
- 막대그래프 필터 강화
- 지도 감지 개선
- 표 감지 개선
...
class Phase342Pipeline:
    """
    Phase 3.4.2 처리 파이프라인 (Hybrid Detection - 정밀 조정)
    
    특징:
    - Layout Detector v3.4.2 (Hybrid Detection)
    - 막대그래프 필터 강화 (min_bar_area: 1200, min_group_width: 250)
    - 지도 감지 개선 (min_text_regions: 1, 키워드 확장)
    - 표 감지 개선 (50~85% 크기 인정)
    """
    ...
    def __init__(self, pdf_processor, layout_detector, vlm_service, storage):
        ...
        Args:
            ...
            layout_detector: LayoutDetectorV342 인스턴스 ⭐
...
JSON 형식으로 반환:
{
  ...
  "data": {...}
    ...
    def process_pdf(
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"🚀 Phase 3.4.2 처리 시작: {pdf_path}")
        ...
        # ========================================
        # Stage 2: Layout Detection v3.4.2 (Hybrid)
        # ========================================
        all_regions = []
        ...
        for page_num, img_data in enumerate(images):
            ...
            logger.info(f"\n[Stage 2] 페이지 {page_num + 1} - Layout Detection v3.4.2 (Hybrid)")
            ...
            try:
                ...
                # ⭐ Layout Detection v3.4.2 (Hybrid)
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"🎉 Phase 3.4.2 처리 완료")
