"""
core/phase34_pipeline.py
PRISM Phase 3.4 - Hybrid Detection Pipeline

✅ Phase 3.4 수정사항:
- LayoutDetectorV34 사용 (v3.3 → v3.4)
- 표/막대그래프/지도 감지 대폭 개선
- 텍스트 과감지 방지
...
class Phase34Pipeline:
    """
    Phase 3.4 처리 파이프라인 (Hybrid Detection)
    
    특징:
    - Layout Detector v3.4 (Hybrid Detection)
    - 표/막대그래프/지도 감지 대폭 개선
    - 텍스트 과감지 방지 (500x500px 블록)
    """
    ...
    def __init__(self, pdf_processor, layout_detector, vlm_service, storage):
        ...
        Args:
            ...
            layout_detector: LayoutDetectorV34 인스턴스 ⭐
...
JSON 형식으로 반환:
{
  ...
  "data": {...}
    ...
    def process_pdf(
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"🚀 Phase 3.4 처리 시작: {pdf_path}")
        ...
        # ========================================
        # Stage 2: Layout Detection v3.4 (Hybrid)
        # ========================================
        all_regions = []
        ...
        for page_num, img_data in enumerate(images):
            ...
            logger.info(f"\n[Stage 2] 페이지 {page_num + 1} - Layout Detection v3.4 (Hybrid)")
