"""
app_phase342.py
PRISM Phase 3.4.2 - Hybrid Detection (Streamlit UI) - 정밀 조정

🔥 Phase 3.4.2 핵심 개선:
1. ✅ 막대그래프 필터 강화 (min_bar_area: 1200, min_group_width: 250)
2. ✅ 지도 감지 개선 (min_text_regions: 1, 키워드 확장)
3. ✅ 표 감지 개선 (50~85% 크기 인정)
4. ✅ 표 내부 요소 제외 (중복 제거 강화)
...
try:
    ...
    from core.layout_detector_v342 import LayoutDetectorV342  # ⭐ v3.4.2
    from core.vlm_service import VLMService
    from core.storage import Storage
    from core.phase342_pipeline import Phase342Pipeline  # ⭐ v3.4.2
...
# ============================================================
st.set_page_config(
    page_title="PRISM Phase 3.4.2 - Precision Tuning",
    page_icon="🎯",
    layout="wide"
)
...
# 헤더
# ============================================================
st.markdown('<div class="main-header">🎯 PRISM Phase 3.4.2<span class="phase-badge">Precision Tuning</span></div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">🔧 막대그래프 필터 강화 | 지도 감지 개선 | 표 감지 개선 | 목표: 7/7/3 영역</div>', unsafe_allow_html=True)

# Phase 3.4.2 개선사항 표시
with st.expander("🎯 Phase 3.4.2 개선사항", expanded=False):
    col1, col2, col3 = st.columns(3)
    
    with col1:
        st.markdown("### 📊 막대그래프 필터 강화")
        st.markdown("**✅ 파라미터 대폭 증가**")
        st.markdown("- min_bar_area: 800 → 1200")
        st.markdown("- min_bar_width: 20 → 30")
        st.markdown("- min_bar_height: 15 → 25")
        st.markdown("- min_group_width: 150 → 250")
        st.markdown("- min_group_height: 60 (신규)")
        st.markdown('<span class="improvement-badge">과감지 방지</span>', unsafe_allow_html=True)
    
    with col2:
        st.markdown("### 🗺️ 지도 감지 개선")
        st.markdown("**✅ 완화 + 키워드 확장**")
        st.markdown("- min_text_regions: 2 → 1")
        st.markdown("- 키워드: 권/도/시")
        st.markdown("  → 수도권/경남권/전라권...")
        st.markdown("- 더 넓은 지역명 인식")
        st.markdown('<span class="improvement-badge">미감지 해결</span>', unsafe_allow_html=True)
    
    with col3:
        st.markdown("### 📋 표 감지 개선")
        st.markdown("**✅ 크기 범위 조정**")
        st.markdown("- 페이지 대비 50~85%")
        st.markdown("- 표 내부 요소 제외")
        st.markdown("- 중복 제거 강화")
        st.markdown('<span class="improvement-badge">정확도 향상</span>', unsafe_allow_html=True)

st.markdown("---")

# 목표 영역 표시
st.markdown("### 🎯 **목표 감지 영역**")
col1, col2, col3 = st.columns(3)
...
try:
    if st.session_state.pipeline is None or st.session_state.get('current_provider') != vlm_provider:
        with st.spinner(f"🔄 {vlm_provider} 초기화 중..."):
            ...
            layout_detector = LayoutDetectorV342()  # ⭐ v3.4.2
            vlm_service = VLMService(provider=vlm_provider)
            storage = Storage()
            
            st.session_state.pipeline = Phase342Pipeline(  # ⭐ v3.4.2
            ...
            st.success(f"✅ Phase 3.4.2 파이프라인 초기화 완료: {vlm_provider}")
...
if uploaded_file is not None:
    ...
    with col3:
        st.metric("🎯 Phase", "3.4.2 (Precision)")
...
if uploaded_file is not None:
    if st.button("🚀 Phase 3.4.2 처리 시작", type="primary", use_container_width=True):
        try:
            # 임시 파일 저장
            temp_dir = Path("temp")
            ...
            # Pipeline 실행
            logger.info(f"🚀 Phase 3.4.2 처리 시작: {uploaded_file.name}")
...
if st.session_state.processing_result is not None:
    ...
    with col2:
        # Markdown 다운로드
        md_content = f"""# PRISM Phase 3.4.2 처리 결과
...
<div style='text-align: center; color: #888; font-size: 0.9rem;'>
    <strong>PRISM Phase 3.4.2 - Precision Tuning</strong><br>
    🎯 막대그래프 필터 강화 | 지도 감지 개선 | 표 감지 개선 | 목표: 7/7/3 영역<br>
